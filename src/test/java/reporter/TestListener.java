...
/**
 * TestListener — TestNG ITestListener that builds a self-contained HTML report.
 *
 * Wired via test.xml:
 *   <listeners>
 *     <listener class-name="reporter.TestListener"/>
 *   </listeners>
 *
...
public class TestListener implements ITestListener {
    ...
    private static class Result {
        ...
        final String errorMessage; // null when PASS
        final String stackTrace;   // null when PASS
    ...
    private Result buildResult(ITestResult r, String status, String msg, String trace) {
        // ITestResult.getTestName() reads the testName="TCXX" attribute from @Test
        // ITestNGMethod.getTestName() was removed in TestNG 7 — do not use it
        String tcId  = r.getTestName() != null ? r.getTestName() : r.getMethod().getMethodName();
        String mName = r.getMethod().getMethodName();
        long   dur   = r.getEndMillis() - r.getStartMillis();
        return new Result(tcId, mName, status, dur, msg, trace);
    }
    ...
    private void writeReport(ITestContext ctx) {
        ...
        for (Result r : results) {
            ...
            if (r.errorMessage != null && !r.errorMessage.isBlank()) {
                String safeMsg   = escape(r.errorMessage);
                String safeTrace = escape(r.stackTrace);
                errorCell = "<details><summary>" + safeMsg + "</summary>"
                        + "<pre>" + safeTrace + "</pre></details>";
            }
            rows.append("<tr>")
...
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .card{border-radius:8px;padding:.65rem 1.2rem;font-size:.88rem;font-weight:600;min-width:110px;text-align:center}
  ...
  th{background:#1a56db;color:#fff;padding:10px 14px;text-align:left;font-size:.82rem;
...
<body>
<header>
  <h1>🏠 PropFind — Selenium Test Report</h1>
  <small>Suite: SUITE_NAME &nbsp;|&nbsp; Run: RUN_TIME</small>
</header>
<div class="summary">
  ...
  <div class="card c-time"><span>TOTAL_MS ms</span>Duration</div>
</div>
<div class="wrap">
<table>
  <thead>
...
""".replace("SUITE_NAME",  escape(suiteName))
   .replace("RUN_TIME",    runTime)
   ...
   .replace("TOTAL_MS",    String.valueOf(totalMs))
